...
def leadership(request):
	return render(request, 'knowledgeprojapp/leadership.html')

def home(request):
	return render(request, 'knowledgeprojapp/index.html')

def plan(request):
	plans= Plan.objects.all()
	context= {'plans':plans}
	return render(request, 'knowledgeprojapp/plan.html', context)

def otherpayment(request):
    return render(request, 'knowledgeprojapp/otherpayment.html')


def about(request):
	return render(request, 'knowledgeprojapp/company.html')


def career(request):
	return render(request, 'knowledgeprojapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				email= user.email
				template= render_to_string('knowledgeprojapp/otpalert.html', {'otp':otp})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
					'Use this OTP code to login to your BitKroft-Capitals!',
					template,
	...
	context={}
	return render(request, 'knowledgeprojapp/login.html')
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if str(otp_obj.otp_code) == str(entered_otp) and not otp_obj.is_expired():
				email= user.email
				template= render_to_string('knowledgeprojapp/loginAlert.html', {'name':user.username})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
				...
				return render(request, 'knowledgeprojapp/verify_otp.html', {'form': form, 'error': 'Invalid or expired OTP'})
	else:
		form = OTPForm()

	return render(request, 'knowledgeprojapp/verify_otp.html', {'form': form})


def privacy(request):
	return render(request, 'knowledgeprojapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'knowledgeprojapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('knowledgeprojapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('knowledgeprojapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to Bitkroft-Capitals',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('knowledgeprojapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Bitkroft-Capitals!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@bitkroft-capitals.com'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'knowledgeprojapp/register.html', context)
...
def dashboard(request):
	...
	return render(request, 'knowledgeprojapp/dashboardtwo.html', context)
...
def admindashboard(request):
	...
	return render(request, 'knowledgeprojapp/adminpage.html', context)


def terms(request):
	return render(request, 'knowledgeprojapp/terms.html')


def realestate(request):
	return render(request, 'knowledgeprojapp/realestate.html')


def crypto(request):
	return render(request, 'knowledgeprojapp/crypto.html')


def forex(request):
	return render(request, 'knowledgeprojapp/forex.html')


def nft(request):
	return render(request, 'knowledgeprojapp/nft.html')


def contact(request):
	return render(request, 'knowledgeprojapp/contact.html')


def stocks(request):
	return render(request, 'knowledgeprojapp/stocks.html')


def agriculture(request):
	return render(request, 'knowledgeprojapp/agriculture.html')


def gold(request):
	return render(request, 'knowledgeprojapp/gold.html')


def retirement(request):
	return render(request, 'knowledgeprojapp/retirement.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.Bitkroft-Capitals.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.Bitkroft-Capitals.com/dashboard"
			})
			...
			template= render_to_string('knowledgeprojapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@Bitkroft-Capitals.com'])
			except BadHeaderError:
				pass
			return redirect(generated_link)
	context={}
	return render(request, 'knowledgeprojapp/deposittwo.html', context)
...
def investmentPlans(request):
	plans= Plan.objects.all()
	context= {'plans':plans}
	return render(request, 'knowledgeprojapp/investmentplans.html', context)

@login_required(login_url='signin')
def noviceDeposit(request):
	if request.method == 'POST':
	...
	context= {}
	return render(request, 'knowledgeprojapp/novicedepositForm.html', context)

@login_required(login_url='signin')
def amateurDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'knowledgeprojapp/amateurDepositForm.html', context)

@login_required(login_url='signin')
def superiorDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'knowledgeprojapp/superiorDepositForm.html', context)

@login_required(login_url='signin')
def mining_loanDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'knowledgeprojapp/mining-loanDepositForm.html', context)

@login_required(login_url='signin')
def amazingofferDeposit(request):
	if request.method == 'POST':
		investment_info= request.POST.get('investment_plan')
		price_amount= request.POST.get('amount')
		pay_currency= request.POST.get('pay_currency')
		...
		return redirect('deposit')
	context= {}
	return render(request, 'knowledgeprojapp/amazingofferDepositForm.html', context)
...
def deposit(request):
	...
	if price_amount and pay_currency:
		...
		headers={'x-api-key':'NM974T0-61H4C8S-QEADPP9-RR2FKMA', 'Content-Type': 'application/json'}
		response= requests.request('POST', url, headers=headers, data=payload)
		res= response.json()
		print(res)
		generated_link= res["invoice_url"]
...
def deposit(request):
	try:
		...
		return HttpResponse('Please try depositing when our packages are available')
	if request.method=='POST':
		client= request.user.client
		client_name= client.first_name
		client_email= request.user.email
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@Bitkroft-Capitals.com'])
	...
	context={}
	return render(request, 'knowledgeprojapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'knowledgeprojapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'knowledgeprojapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'knowledgeprojapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'knowledgeprojapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'knowledgeprojapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'knowledgeprojapp/referralprofilestwo.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'knowledgeprojapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('knowledgeprojapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('knowledgeprojapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'knowledgeprojapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('knowledgeprojapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'knowledgeprojapp/account_settings.html', context)
...
def create_bonus(request):
				...
				try:
					...
					if bonus_creation:
						...
						if client_email:
							...
							template= render_to_string('knowledgeprojapp/bonus_email.html', {'amount':amount, 'code':code})
							plain_message= strip_tags(template)
							emailmessage= EmailMultiAlternatives(
								'Bonus Notification',
								plain_message,
	...
	return render(request, 'knowledgeprojapp/create_bonus.html', context)
...
def use_bonus(request):
		...
		if client_request_code:
			try:
				...
				if code_info.is_valid:
					...
					template= render_to_string('knowledgeprojapp/usebonus_email.html', {'name':code_info.client})
					plain_message= strip_tags(template)
					emailmessage= EmailMultiAlternatives(
						'Bonus Code Used',
						plain_message,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					send_mail(code_info.client, "A client with username: {} has used their bonus code of {} usd. Money has been credited to their account".format(code_info.client, amount),settings.EMAIL_HOST_USER, ['info@Bitkroft-Capitals.com'])
					return redirect('dashboard')
				else:
	...
	return render(request, 'knowledgeprojapp/use_bonus.html')
...
def notifications(request):
	...
	return render(request, 'knowledgeprojapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('knowledgeprojapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('knowledgeprojapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@Bitkroft-Capitals.com'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'knowledgeprojapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'knowledgeprojapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('knowledgeprojapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('knowledgeprojapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@Bitkroft-Capitals.com'])
			except BadHeaderError:
				pass
	...
	context= {'packages': packages}
	return render(request, 'knowledgeprojapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@Bitkroft-Capitals.com'])
	...
	return render(request, 'knowledgeprojapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'knowledgeprojapp/portfolio.html', context)